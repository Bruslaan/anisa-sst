...
import { transcribeAudio as ItranscribeAudio } from "./openAi/transcribe-audio";
...
export const askAnisa = async (
  ...
  try {
    ...
    await Supabase.saveMessageToDatabase(
      userId,
      createMessage(userId, "assistant", aiContent, aiResponse.image_url)
    );
...
export namespace Anisa {
  ...
  export const transcribeAudio = ItranscribeAudio;
}
