...
export const optimizeContextWindow = (
  ...
  if (estimatedTokens > MAX_CONTEXT_TOKENS) {
    const maxMessagesForTokens = Math.floor(
      MAX_CONTEXT_TOKENS / AVERAGE_TOKENS_PER_MESSAGE
    );
...
export const buildContextualHistory = (
  ...
  const messageHistory: EasyInputMessage[] = optimizedMessages
    .filter((msg) => msg.content || msg.media_url?.type === "image")
    .map((msg) => {
      const isImage = msg.media_url?.type === "image";
      const aisMessage: EasyInputMessage = isImage
        ? {
            role: "user",
            content: [
              {
                type: "input_text",
                text: msg.content ?? "Image provided",
              },
              {
                type: "input_image",
                detail: "low",
                image_url: msg.media_url?.url,
              },
            ],
          }
        : {
